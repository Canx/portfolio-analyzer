...
def render_main_content(df_metrics, daily_returns, portfolio, mapa_isin_nombre):
    ...
    pesos = st.session_state.get("carteras", {}).get(st.session_state.get("cartera_activa"), {}).get("pesos", {})
    if pesos and sum(pesos.values()) > 0:
        ...
        fig_pie = px.pie(df_pie, names='Fondo', values='Peso', title='Composición Actual de la Cartera', hole=.3)
    ...
    if daily_returns.empty or len(daily_returns) < 2:
        st.warning("No hay suficientes datos históricos en el periodo seleccionado para generar los gráficos.")
        st.info("💡 Prueba a seleccionar un horizonte temporal más largo (ej. '1y' o 'max').")
    else:
        ...
        if not df_metrics.empty:
            ...
            fig_risk = px.scatter(fondos_metrics, x="volatility_ann_%", y="annualized_return_%", text="nombre", hover_name="nombre", title="Riesgo vs. Retorno de los Fondos")
            fig_risk.update_traces(textposition='top center')
            cartera_metrics = df_metrics[df_metrics["nombre"].str.startswith('💼')]
            if not cartera_metrics.empty:
                fig_risk.add_trace(go.Scatter(x=cartera_metrics["volatility_ann_%"], y=cartera_metrics["annualized_return_%"], mode='markers', marker=dict(color='red', size=15, symbol='star'), name=cartera_metrics.iloc[0]["nombre"]))
