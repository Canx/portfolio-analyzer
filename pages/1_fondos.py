...
st.write(
    "Aquí puedes ver, filtrar, actualizar, añadir nuevos fondos al catálogo y asignarlos a tu cartera activa."
)
...
with st.expander("➕ Añadir nuevo fondo al catálogo por ISIN"):
    with st.form("form_add_fund_explorer"):
        new_isin = (
            st.text_input(
                "Introduce un ISIN para buscarlo", placeholder="Ej: IE00B4L5Y983"
            )
            .strip()
            .upper()
        )
...
selected_domicilios = st.sidebar.multiselect(
    "Filtrar por Domicilio", domicilios, default=[]
)
...
selected_max_ter = st.sidebar.slider(
    "TER Máximo (%)",
    0.0,
    float(np.ceil(max_ter_value)),
    value=float(np.ceil(max_ter_value)),
    step=0.1,
)
...
df_filtered = df_filtered[
    (df_filtered["ter"] <= selected_max_ter) | (df_filtered["ter"].isna())
]
if search_term:
    df_filtered = df_filtered[
...
if active_portfolio_name:
    isins_in_active_portfolio = (
        st.session_state.carteras.get(active_portfolio_name, {}).get("pesos", {}).keys()
    )
    ...
    with col8:
        ...
        if row["isin"] in isins_in_active_portfolio:
            ...
            if st.button("➕ Añadir", key=f"add_explorer_{row['isin']}"):
                if active_portfolio_name:
                    st.session_state.carteras[active_portfolio_name]["pesos"][
                        row["isin"]
                    ] = 0

                    # Rebalanceamos los pesos
                    pesos_actuales = st.session_state.carteras[active_portfolio_name][
                        "pesos"
                    ]
                    num_fondos = len(pesos_actuales)
                    peso_base = 100 // num_fondos
                    st.session_state.carteras[active_portfolio_name]["pesos"] = {
                        isin: peso_base for isin in pesos_actuales.keys()
                    }

                    resto = 100 - sum(
                        st.session_state.carteras[active_portfolio_name][
                            "pesos"
                        ].values()
                    )
                    if num_fondos > 0:
                    ...
                    st.warning(
                        "Ve a 'Análisis de Cartera' para crear o seleccionar una cartera primero."
                    )
